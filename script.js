...
function tombol() {
    ...
    if (fungsi == 2) { By.innerHTML = "&#128140; Sesuatu"; }
}
...
function aktipesan5() {
    ...
    new TypeIt("#pesan5", {
        ...
        afterComplete: function () {
            ...
            setTimeout(aktipesanf, 800);
        },
    }).go();
}

vketikf = pesanf.innerHTML;
pesanf.innerHTML = "";
...
document.getElementById("lv1").onclick = function () {
    lv1.classList.add('slideUpAndFadeOut');
    lv1.style = "opacity:0";
    slov += 1;
    this.onclick = null;
    checkslov();
...
document.getElementById("lv2").onclick = function () {
    lv2.classList.add('slideUpAndFadeOut');
    lv2.style = "opacity:0";
    slov += 1;
    this.onclick = null;
    checkslov();
...
document.getElementById("lv3").onclick = function () {
    lv3.classList.add('slideUpAndFadeOut');
    lv3.style = "opacity:0";
    slov += 1;
    this.onclick = null;
    checkslov();
...
document.getElementById("lv4").onclick = function () {
    lv4.classList.add('slideUpAndFadeOut');
    lv4.style = "opacity:0";
    slov += 1;
    this.onclick = null;
    checkslov();
...
async function pertanyaan() {
    var { isConfirmed: prtanya } = await swals.fire({
        ...
        didOpen: () => {
            ...
            const moveButtonAway = (event) => {
                const rect = cancelButton.getBoundingClientRect();
                const buttonX = rect.left + rect.width / 2;
                const buttonY = rect.top + rect.height / 2;
                const distX = buttonX - event.clientX;
                const distY = buttonY - event.clientY;
                const distance = Math.sqrt(distX * distX + distY * distY);

                // Move the button away if the cursor is too close
                if (distance < 100) {
                    const angle = Math.atan2(distY, distX);
                    const moveX = Math.cos(angle) * 150;
                    const moveY = Math.sin(angle) * 150;
                    const newX = buttonX + moveX;
                    const newY = buttonY + moveY;
                    ...
                    cancelButton.style.transform = `translate(${finalX - buttonX}px, ${finalY - buttonY}px)`;
                } else {
                    cancelButton.style.transform = 'translate(0, 0)';
                }
            };
